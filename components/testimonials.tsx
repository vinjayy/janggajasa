      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          ...
          <p className="mt-4 max-w-2xl mx-auto text-xl text-gray-500">
            Testimoni dari pelanggan yang telah menggunakan layanan Janggajasa
          </p>
        </div>
        ...
        <div className="relative">
                ...
                <div key={testimonial.id} className="w-full md:w-1/2 lg:w-1/3 flex-shrink-0 px-4">
                  <Card className="h-full">
                    <CardContent className="p-6">
                      ...
                      <p className="text-sm font-medium text-emerald-600">Layanan: {testimonial.service}</p>
                    </CardContent>
                  </Card>
                </div>
              ))}
          ...
          <button
            className="absolute left-0 top-1/2 transform -translate-y-1/2 -translate-x-1/2 w-12 h-12 rounded-full bg-white/80 backdrop-blur-sm shadow-lg z-10 hidden md:flex items-center justify-center transition-all duration-300 hover:bg-emerald-500 hover:text-white hover:scale-110"
            onClick={prevSlide}
          >
            <svg
              width="24"
          ...
          <button
            className="absolute right-0 top-1/2 transform -translate-y-1/2 translate-x-1/2 w-12 h-12 rounded-full bg-white/80 backdrop-blur-sm shadow-lg z-10 hidden md:flex items-center justify-center transition-all duration-300 hover:bg-emerald-500 hover:text-white hover:scale-110"
            onClick={nextSlide}
          >
            <svg
              width="24"
        ...
        <div className="flex justify-center mt-8 space-x-2 md:hidden">
          {[...Array(testimonials.length - maxVisibleItems + 1)].map((_, i) => (
            <button
              key={i}
              className={`h-2 w-2 rounded-full ${currentIndex === i ? "bg-emerald-600" : "bg-gray-300"}`}
